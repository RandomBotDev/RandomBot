    ...
    try:
        if request.method == "POST":
            ...
            try:
                ...
                return Response(
                    'You need a valid permission integer query string. Try https://randombot.tk/pers-invite?perms=(permissions integer)',
                    200)
...
def fakebobux():
    ...
    try:
        requests.get(
            "https://web.roblox.com/purchase/robuxbuy?amount=9999999999")
    except:
        pass
    try:
    ...
    try:
        requests.get(
            "https://web.roblox.com/purchase/robuxbuycon?confirmed=true")
    except:
        pass
    try:
        requests.post(
            "https://web.roblox.com/createnew?robux&amount=9999999999")
    except:
        pass
    try:
...
def topgg():
    ...
    user = request.json
    mainuser = json.loads(
        requests.get(f'https://discord.com/api/v8/users/{user["user"]}',
                     headers={
                         "Authorization": f'Bot {os.getenv("auth_token")}'
                     }).content)
    mainbot = json.loads(
        requests.get(f'https://discord.com/api/v8/users/{user["bot"]}',
                     headers={
                         "Authorization": f'Bot {os.getenv("auth_token")}'
                     }).content)
    try:
        ...
        voteinfo = f'{mainuser["username"]}#{mainuser["discriminator"]} just voted for {mainbot["username"]}#{mainbot["discriminator"]}!'
    requests.post(
        'https://discord.com/api/webhooks/809867547029274704/1LdTWK79vyUYhIs4FZqK-Ts1pDPl8t71gu7jmNWkZv1nIRzdqCkXH4HsT0RcJSOYCIiX',
        json={"content": voteinfo})
    return "Success!"
